...
Status EncapsulateClusters(
  ...
  SubgraphExtractionResults results;
  TF_RETURN_IF_ERROR(ExtricateSubgraph(graph, results));

  const std::map<int, std::string>& device_name_map = results.device_name_map;
  const std::map<int, std::string>& backend_name_map = results.backend_name_map;
  const std::map<int, std::vector<std::tuple<int, int, DataType>>>&
      cluster_input_map = results.cluster_input_map;
  const std::map<int, std::vector<DataType>>& cluster_output_dt_map =
      results.cluster_output_dt_map;
  const std::map<std::tuple<int, int>, std::tuple<int, int>>& output_remap_map =
      results.output_remap_map;
  const std::map<std::tuple<int, std::string, int>, string>& input_rename_map =
      results.input_rename_map;

  for (int i = 0; i < NGraphClusterManager::GetNumClusters(); i++) {
    auto gdef = NGraphClusterManager::GetClusterGraph(i);
    cout << "For cluster " << i << ": " << gdef->node_size() << "\n";
  }

  // Pass 3: Create encapsulation nodes for all clusters.
  for (const auto& kv : device_name_map) {
    int cluster_idx = kv.first;
    string cluster_backend = backend_name_map.at(cluster_idx);
    ...
    auto it = cluster_input_map.find(cluster_idx);
    if (it != cluster_input_map.end()) {
    ...
    NodeBuilder nb =
        NodeBuilder(ss.str(), "NGraphEncapsulate")
            ...
            .Attr("Tresults", cluster_output_dt_map.at(cluster_idx))
            .Attr("ngraph_graph_id", graph_id)
            .Device(device_name_map.at(cluster_idx))
            .Input(inputs);
    ...
    n->set_assigned_device_name(device_name_map.at(cluster_idx));

  ...
  for (auto node : input_graph->op_nodes()) {
    ...
    if (fetch_node) {
      ...
      if (node->num_outputs()) {
        ...
        Node* identityN_node;
        cout << "Made IdN with name: " << node->name() << "\n";
        ...
        cout << "NGTF_OPTIMIZER: num outputs " << node->num_outputs() << "\n";
        cout << "NGTF_OPTIMIZER: Cannot construct an IdentityN node\n";
      }
    }
  }
  return Status::OK();

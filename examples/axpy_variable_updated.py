...
# Define the data
a = tf.constant(np.full((2, 2), 1.5, dtype=np.float32), name='alpha')
x = tf.get_variable('x', [2, 2], initializer=tf.zeros_initializer)
y = tf.constant(np.full((2, 2), 1.0, dtype=np.float32), name='y')
